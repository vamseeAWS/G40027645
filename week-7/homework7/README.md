...
To execute and validate the lab environment for this Flask + Docker + NGINX security hardening project, a structured Makefile was used to streamline and enforce repeatable, secure operational workflows. This section describes the setup and testing process step-by-step, as implemented during the lab exercises.

## 7.1 Pre-Build Security Validation

Before building the Docker image, static code analysis and dependency vulnerability assessments were performed:

- **Static Analysis with Bandit:**
  Using the `check` target in the Makefile, Bandit scanned the Python source code for common security issues such as use of insecure functions (e.g., `eval`, `subprocess`) and improper permissions.
...
- **Dependency Audit with pip-audit:**
  The same `check` command invoked `pip-audit` to detect known vulnerabilities in the Python dependencies listed in `requirements.txt`.

These steps enforced security hygiene at the earliest stage of the container lifecycle.

## 7.2 Host Hardening Validation

- **Docker Bench for Security:**
  The `host-security` target ran Docker's `docker-bench-security` script:

  ```bash
  make host-security
  ```

This validated the Docker daemon and host configurations against CIS benchmarks.

## 7.3 Docker Image Build and Vulnerability Scanning

- **Build Docker Image with Security Gate:**
...
This ensured that no high-severity CVEs were present in the final container image.

## 7.4 Stack Launch and Validation
...
- **Test Routes:**
  ...
  - `http://localhost:8080/ping?ip=8.8.8.8`
  - `http://localhost:8080/calculate?expr=5%2B3`
